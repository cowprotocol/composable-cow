...
The initcode of the official deployments, one file per contract, with `manifest.json` recording each
salt and resulting address. Read back from the deployment transactions, not compiled from this
repository: a fresh build produces a different metadata hash and therefore a different address
